...
class _ProductivityScreenState extends State<ProductivityScreen> {
  ...
  void _loadHabits() async {
    ...
    String? savedData = prefs.getString('productivity_habits');
    if (savedData != null) {
      setState(() {
      ...
      setState(() {
        habits = [
          Habit(name: "Solve 1 LeetCode Question", progress: 0, goal: 1),
          Habit(name: "Read for 30 minutes", progress: 0, goal: 30),
          Habit(name: "Write Journal Entry", progress: 0, goal: 1),
        ];
      });
      _saveHabits();
    }
    ...
    builder: (context) {
      return StatefulBuilder( // Allows live updates inside the dialog
        builder: (context, setState) {
          return AlertDialog(
            title: Text("Add New Productivity Habit"),
            content: Column(
